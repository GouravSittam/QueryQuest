...
const ReadQuiz = ({ data }) => {
  ...
  const handleSubmit = () => {
    const newScore = quizData.reduce((acc, question, index) => {
      if (question.type === "READ_ALONG") {
        return userAnswers[index] ? acc + 1 : acc // Consider it correct if any answer is provided
      } else {
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto">
      ...
      <CardContent>
        {currentQuestionData.type === "READ_ALONG" ? (
          <ReadAlongQuestion question={currentQuestionData} onAnswer={handleAnswer} />
        ) : (
